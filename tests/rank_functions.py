...
"""Compute per-function metrics (NLOC and cyclomatic complexity) for C files."""
...
def main():
    ...
    required_cols = {"source_file", "function_name"}
    if not required_cols.issubset(df.columns):
        raise SystemExit(
            f"Input CSV must contain columns {required_cols}. Found: {set(df.columns)}"
        )
